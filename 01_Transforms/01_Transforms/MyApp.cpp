...
bool CMyApp::Init()
{
	...
	{ 
		...
		{glm::vec3(-1, -1, 1), glm::vec3(1, 0, 0)},
		{glm::vec3(1 , -1, 1), glm::vec3(0, 1, 0)},
		{glm::vec3(0 ,  1, 0), glm::vec3(0, 0, 1)}, // csúcs
...
void CMyApp::Render()
{
	...
	*/
	...
	float scaler = sinf(SDL_GetTicks() / 1000.f) * 0.5f + 1.f;

	m_matWorld = glm::translate<float>(glm::vec3(x_offset1, 0, 0)) *
				 glm::scale<float>(glm::vec3(scaler, 1, scaler)) *
				 glm::rotate<float>(static_cast<float>(SDL_GetTicks()) / 10.f * (M_PI / 180.f), glm::vec3(0, 1, 0));
				 
	m_time = sinf(SDL_GetTicks() / 1000.f) * 0.5f + 0.5f;
	...
	glUniform1fv(m_loc_time, 1, &m_time);
	...
	*/
	...
	m_matWorld = glm::translate<float>(glm::vec3(x_offset2, 0, 0)) *
				 glm::scale<float>(glm::vec3(scaler, 1, scaler)) *
				 glm::rotate<float>((-1.f) * static_cast<float>(SDL_GetTicks()) / 10.f * (M_PI / 180.f), glm::vec3(0, 1, 0));
...
{
	double r = sqrt(pow(look_at_from_z, 2) + pow(look_at_from_x, 2));
	// A fok miatt ugrik hirtelen az A D, dunno how to fix
	...
	case SDLK_a:
		...
		deg-=5;
		look_at_from_x = r * cos(deg * (M_PI/180));
		look_at_from_z = r * sin(deg * (M_PI / 180));
		break;
	case SDLK_s:
